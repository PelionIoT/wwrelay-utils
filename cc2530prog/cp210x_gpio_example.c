...
int main(int argc, char *argv[])
{
	if ( argc < 2 ){ // argc should be 2 for correct execution
    		...
    		printf("usage: %s <get|set|toggle>\n",argv[0]);
    		printf("  %s set g1low //sets g1 pin low",argv[0]);
    		printf("  %s set g2high //sets g2 pin high",argv[0]);
    		printf("  %s set alllow //sets all pins low",argv[0]);
    		printf("  %s set allhigh //sets all pins high",argv[0]);
		...
		printf("Original GPIO = %#08x result = %i \n",gpio ,result);
	...
	// Now check the new GPIO value
			...
			else if (strcmp(argv[2], "alllow") == 0){
				gpio = alllow;
			}
			else if (strcmp(argv[2], "allhigh") == 0){
				gpio = allhigh;
			}
			printf("about to set: %#08x \n",gpio);
			...
			printf("would normally do what you want.  Someone write me\n");
		}
		close(fd);
		return 0;		
	}
