...
WWEEPROM.prototype.set = function(key, value, serial) {
	...
	return new Promise(function(resolve, reject) {
		lookup = self.Layout[key]; //eval("self.Layout." + key);
		// if (lookup.len == value.length) {
		// console.log("calling setText ", lookup);
		try {
...
WWEEPROM.prototype.erase = function(page, callback) {
	return new Promise(function(resolve, reject) {
		try {
			var emptyString = new Buffer(256);
			emptyString.fill(0xFF);
			at24.erase(page, 0, emptyString.toString().split(''), function(err, success) {
...
WWEEPROM.prototype.get = function(key, serial) {
	...
	return new Promise(function(resolve, reject) {
		lookup = self.Layout[key]; //eval("self.Layout." + key);
		var totalchars = (lookup.memaddr/1 + lookup.len/1) || 256;
