...
setRange() {
	...
	if [[ $fieldlengthsize -eq 256 && $fieldDatasize -lt 256 ]]; then
		...
		log error "Failing before write your string: $fieldData ($newstring) size = $fieldDatasize and the slot size is $fieldlengthsize"
		exit
	fi
	if [[ "$platform" != "softRelay" ]]; then
		for (( pnum = $start ; pnum <= $end ; pnum++ )); do
...
setJson(){
	...
	for field in "${fields[@]}"; do
		...
		if [[ "$f" != "" ]]; then
			...
			log warn "Missing field:\t$field ($f)"
		fi
	done
}
