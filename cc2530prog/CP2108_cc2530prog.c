...
#define alllow		0x0000ffff

#define M1Rlow		G8low
#define M1Rhigh	G8high
#define M1Dlow		G9low
#define M1Dhigh	G9high
#define M1Clow		G11low
#define M1Chigh	G11high
#define M2Rlow		G4low
#define M2Rhigh	G4high
#define M2Dlow		G5low
#define M2Dhigh	G5high	
#define M2Clow		G7low
#define M2Chigh	G7high
#define M3Rlow		G0low
#define M3Rhigh	G0high
#define M3Dlow		G1low
#define M3Dhigh	G1high
#define M3Clow		G3low
#define M3Chigh	G3high 
#define M4Rlow		G12low
#define M4Rhigh	G12high
#define M5Rlow		G13low
#define M5Rhigh	G13high
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "f:rlc:ivPs:d:e:k:")) > 0) {
		switch (opt) {
		case 'f':
			firmware = optarg;
			break;
		...
		case 's':
			...
			fprintf(stdout, "Got relay version: %d\n", relay_version);
			break;
		case 'd':
	...
	if (read(f, temp, fwsize) < 0) {
		fprintf(stderr, "premature end of read\n");
		goto out_free;
	}

	hex_decode(temp, fwsize, fwdata);

	fwsize = fwsize / 2;

	//Test if the file is read character wise or byte wise
	// int q = 0;
	// for(q=0;q<fwsize;q++) {
	// 	printf("%02x ", fwdata[q]);
	// }
